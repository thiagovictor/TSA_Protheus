	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		Else
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If !lNatOper
					If Empty(cNatOper)
						cNatOper := Tabela("13",SF4->F4_CF) //Alltrim((cAliasSD2)->D2_CF)
					Else
				...
				Else	
					...
					If Empty(cNatOper)
						cNatOper := Tabela("13",SF4->F4_CF)  //AllTrim(SubStr(SX5->X5_DESCRI,1,55))
	    			EndIf
	    		EndIf 
				//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
					...
					If (cAliasSD2)->D2_TIPO $ "DBN"
						...
						If MsSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
							...
							If cChave <> (SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,IIF(SD1->D1_TIPO $ "DB",IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA1->A1_CGC),IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA2->A2_CGC)),SM0->M0_ESTCOB,SF1->F1_ESPECIE,SF1->F1_CHVNFE) .OR. (cAliasSD2)->D2_ITEM <> cItemOr
								aadd(aNfVinc,{Iif(!Empty(SD1->D1_DATADOC),SD1->D1_DATADOC,SD1->D1_EMISSAO),SD1->D1_SERIE,SD1->D1_DOC,IIF(SD1->D1_TIPO $ "DB",IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA1->A1_CGC),IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA2->A2_CGC)),SM0->M0_ESTCOB,SF1->F1_ESPECIE,SF1->F1_CHVNFE})
								cChave := (Iif(!Empty(SD1->D1_DATADOC),SD1->D1_DATADOC,SD1->D1_EMISSAO),SD1->D1_SERIE,SD1->D1_DOC,IIF(SD1->D1_TIPO $ "DB",IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA1->A1_CGC),IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA2->A2_CGC)),SM0->M0_ESTCOB,SF1->F1_ESPECIE,SF1->F1_CHVNFE)
							EndIf	
...
For nX=1  to Len(aNfVinc)
	...
	If !ConvType(cNfVinc, Len(cNfVinc)) $ cString
...
If "1"$cModXml //BH - ABRASF
	...
	cString += '<CodigoTributacaoMunicipio>'+ConvType(aISSQN[07],7)+'</CodigoTributacaoMunicipio>'
