...
Local cEol:=''
	  ...
	  //	If Empty(SRC->RC_PEDCOM)
		 ...
		 //	If !GerPcom()		
			...
			//Else
			  //	If !Empty(QTMP1->RA_DEMISSA)
				 //	FGravaCrit(QTMP1->RA_MAT,cEol+"Data de demissão/cancelamento dia: "+Dtoc(QTMP1->RA_DEMISSA)+cEol)
				//Endif
			  //	FGravaCrit(QTMP1->RA_MAT,"Pedido de Compras Nro:"+SRC->RC_PEDCOM+" Gerado Com sucesso",SRC->RC_PEDCOM+cEol)	
			//Endif 
		//Else
		  //	FGravaCrit(QTMP1->RA_MAT,"Já Existe o Pedido Nro:"+SRC->RC_PEDCOM+" na Competência "+cCompet,,'N')		
		//Endif
...
//	cQuery1+=" WHERE CNA.D_E_L_E_T_<>'*' AND CNA_MAT = '"+QTMP1->RA_MAT+"'"  
...
//	If !EOF("QSCON")
  //		dbSelectArea("CN9")
	 //	dbSeek(QSCON->CNA_FILIAL+QSCON->CNA_CONTRA+QSCON->CNA_REVISA)
  ...
  //	dbCloseArea("QSCON")  
	...
	While !Eof("QPCOM") 
		...
		if !SB1->(dbSeek(Xfilial("SB1")+cProdSC7))
			FGravaCrit(SRC->RC_MAT,cEol+"Produto("+Alltrim(cProdSC7)+") Inválido para a SubConta:"+Alltrim(Substring(QPCOM->ZZ_FPCUSTO,10,5)),,'N')
			lMsErroAuto:=.T.
		endif   
	  ...
	  if !lMsErroAuto
		...
		//Acerta o Arredontamento do Item no pedido
			if lMsErroAuto
				...
				aParaPed:={cNumPed,cMatric,SRA->RA_FOR,cLojaF,cMesRef,'100'}
