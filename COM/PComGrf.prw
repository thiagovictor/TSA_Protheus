...
/*
+-----------------------------------------------------------------------+
¦Programa  ¦PCOMGRF    ¦ Autor ¦ Gilson Lucas          ¦Data ¦09.05.2011¦
+----------+------------------------------------------------------------¦
...
Private oFontCab  := TFont():New("Courier New",09,09,,.T.,,,,.T.,.F.)
...
While !(cAliasTemp)->(Eof()) .And. Iif(lImpAuto , (cAliasTemp)->(C7_FILIAL+C7_NUM) == cNumPed,.T.)
   ...
   For nXi:=1 To MLCount(Alltrim((cAliasTemp)->C7_DESCRI),nQuebDesc)
       nTotItens++
   Next nXi
...
While !(cAliasTemp)->(Eof()) .And. Iif(lImpAuto , (cAliasTemp)->(C7_FILIAL+C7_NUM) == cNumPed,.T.)
   ...
   If cChaveOld # cChaveNew 
      aItensPC  := {}
      aDadosFor := {SA2->A2_COD,SA2->A2_NOME,SA2->A2_END,'',SA2->A2_CONTATO,SA2->A2_TEL,SA2->A2_EMAIL,Iif(Len(Alltrim(SA2->A2_CGC))==14,Transform(SA2->A2_CGC,"@R 99.999.999/9999-99"),Transform(SA2->A2_CGC,"@R 999.999.999-99"))}
   ...
   For nXi:=1 To MLCount(Alltrim((cAliasTemp)->C7_PRODUTO)+" - "+Alltrim((cAliasTemp)->C7_DESCRI),nQuebDesc,,.T.)
       If nXi == 1 // na Primeira linha fica as quantidades e valores
          ...
          Aadd(aItensPC,{(cAliasTemp)->C7_ITEM,;
                         ...
                         '',;      
                         Transform((cAliasTemp)->C7_PICM,"@E 99.99"),;
                         Transform((cAliasTemp)->C7_IPI,"@E 99.99"),;
   ...
   If (cChaveOld # cChaveNew)
      ItensPC(@oPrint,@nLinha,@aItensPC,cAliasTemp,aDadosFor,cChaveOld,aDadCab,@nPgAtu,nTotPg,cCondPad,lPedLib,cTpFrete,cDetCot)
   EndIf
End
...
RodaPC(oPrint,@nLinha,cChaveOld,nTotPed,cCondPad,lPedLib,cCondPad,cTpFrete,cDetCot)
...
//oPrint:Box(0710,0050,860,2300) //Box Inf Fornecedor
//oPrint:Line(0710,1670,860,1670)
...
Static Function ItensPC(oPrint,nLinha,aItensPC,cAliasTemp,aDadosFor,cChaveOld,aDadCab,nPgAtu,nTotPg,cCondPad,lPedLib,cTpFrete,cDetCot)
*********************************************************************************
* 
***        
Local aAuxi   := {}
...
oPrint:Say(nLinha,0185   ,OemToAnsi("QTDE"),oFontCab,100)
oPrint:Say(nLinha,0310   ,OemToAnsi("UND"),oFontCab,100)
oPrint:Say(nLinha,0420   ,OemToAnsi("Descrição do Fornecimento"),oFontCab,100)

oPrint:Say(nLinha,1270   ,OemToAnsi("C.Fiscal"),oFontCab,100)
oPrint:Say(nLinha,1520   ,OemToAnsi("Pr.Unit."),oFontCab,100)
oPrint:Say(nLinha,1835   ,OemToAnsi("Impostos"),oFontCab,100)
oPrint:Say(nLinha+50,1680,OemToAnsi("ICMS R."),oFontIte,100)
oPrint:Say(nLinha+50,1825,OemToAnsi("ICMS"),oFontIte,100)
oPrint:Say(nLinha+50,1920,OemToAnsi("IPI"),oFontIte,100)
oPrint:Say(nLinha+50,2020,OemToAnsi("%"),oFontIte,100)
oPrint:Say(nLinha,2070   ,OemToAnsi("Pr Final"),oFontIte,100)
...
For nXi := 1 To 21
    ...
    If Len(aItensPC) >= nXi
       ...
       oPrint:Say(nLinha+50,0185   ,aItensPC[nXi][02],oFontIte,100)
       oPrint:Say(nLinha+50,0310   ,aItensPC[nXi][03],oFontIte,100)
       oPrint:Say(nLinha+50,0420   ,aItensPC[nXi][04],oFontIte,100)
       oPrint:Say(nLinha+50,1270   ,aItensPC[nXi][05],oFontIte,100)
       oPrint:Say(nLinha+50,1520   ,aItensPC[nXi][07],oFontIte,100)
       oPrint:Say(nLinha+50,1680   ,aItensPC[nXi][06],oFontIte,100)
       oPrint:Say(nLinha+50,1915   ,aItensPC[nXi][08],oFontIte,100)
       oPrint:Say(nLinha+50,1825   ,aItensPC[nXi][09],oFontIte,100)
       oPrint:Say(nLinha+50,1920   ,aItensPC[nXi][10],oFontIte,100)
       oPrint:Say(nLinha+50,2020   ,'%',oFontIte,100)
       oPrint:Say(nLinha+50,2060   ,aItensPC[nXi][11],oFontIte,100)
    EndIf
    oPrint:Line(nLinha,050,nLinha,2300)
Next nXi         

nLinha += 100
oPrint:Box(nLinha ,0050,nLinha+120,2300) //Box Itens
oPrint:Line(nLinha,0450,nLinha+120,0450) // Linha vertical
oPrint:Line(nLinha,1570,nLinha+120,1570) // Linha vertical
oPrint:Line(nLinha,1950,nLinha+120,1950) // Linha vertical
nLinha += 010
oPrint:Say(nLinha,0060   ,OemToAnsi("ESPECIFICAÇÃO"),oFontCab,100)
oPrint:Say(nLinha,0460   ,OemToAnsi("Conforme descrito acima."),oFontIte,100)
oPrint:Say(nLinha,1660   ,OemToAnsi("DESTINAÇÃO DO"),oFontCab,100)
...
oPrint:Say(nLinha,1660   ,OemToAnsi("MATERIAL"),oFontCab,100)
...
Static Function RodaPC(oPrint,nLinha,cNumPed,nTotPed,cCondPad,lPedLib,cCondPad,cTpFrete,cDetCot)
*********************************************************************************
* 
***
Local aAssina    := {}
...
oPrint:Say(nLinha,0800 ,OemToAnsi("CONDIÇÕES COMERCIAIS"),oFontCab,100)
nLinha += 50
...
oPrint:Say(nLinha,0060 ,OemToAnsi("Valor Total:"),oFontCab,100)
...
oPrint:Line(nLinha,050,nLinha,2300)
oPrint:Say(nLinha,0060 ,OemToAnsi("PRAZO DE ENTREGA(Dias úteis ou corridos/Data):  "+cPrzEntreg),oFontCab,100)


nLinha += 50
oPrint:Line(nLinha,050,nLinha,2300)
oPrint:Line(nLinha,1200,nLinha+050,1200) // Linha vertical
oPrint:Say(nLinha,0060 ,OemToAnsi("COND. DE PAGAMENTO: ")+cCondPad,oFontCab,100) 
oPrint:Say(nLinha,1250 ,OemToAnsi("COND. DE ENTREGA: ")+Iif(cTpFrete=="C","CIF","FOB"),oFontCab,100) 
nLinha += 50
oPrint:Line(nLinha,050,nLinha,2300)
oPrint:Say(nLinha,0060 ,OemToAnsi("Observação: "),oFontCab,100) 
...
For nXi := 1 To Len(aAssina)
    Do Case
       Case nXi == 1
            ...
            oPrint:Say(nLinha+230,0065,aAssina[nXi,4],oFontDad2,100)            
       ...
       Case nXi == 2
            ...
            oPrint:Say(nLinha+230,0400,aAssina[nXi,4],oFontDad2,100)            
            oPrint:SayBitmap(nLinha+260,0406,aAssina[nXi,2],300,120)
       Case nXi == 3
            If aAssina[nXi,1] == "NãO SE APLICA"  .or. aAssina[nXi,1]== 'Não Aprovado'
               oPrint:Say(nLinha+030,0810,aAssina[nXi,1],oFontDad2,100)           
            Else   
               oPrint:Say(nLinha+005,0810,Left(aAssina[nXi,1],10),oFontDad2,100)
               oPrint:Say(nLinha+055,0810,Alltrim(SubStr(aAssina[nXi,1],11,Len(aAssina[nXi,1]))),oFontDad2,100)
            EndIf
            
            oPrint:Say(nLinha+130,0810,OemToAnsi("DATA: ")+Dtoc(aAssina[nXi,3]),oFontDad2,100)            
            oPrint:Say(nLinha+230,0820,aAssina[nXi,4],oFontDad2,100)
            oPrint:SayBitmap(nLinha+260,0820,aAssina[nXi,2],300,120)
       Case nXi == 4
            If aAssina[nXi,1] == "NãO SE APLICA"  .or. aAssina[nXi,1]=='Não Aprovado'
               oPrint:Say(nLinha+030,1230,aAssina[nXi,1],oFontDad2,100)           
            Else   
               oPrint:Say(nLinha+005,1230,Left(aAssina[nXi,1],10),oFontDad2,100)
               oPrint:Say(nLinha+055,1230,Alltrim(SubStr(aAssina[nXi,1],11,Len(aAssina[nXi,1]))),oFontDad2,100)
            EndIf
            oPrint:Say(nLinha+130,1233,OemToAnsi("DATA: ")+Dtoc(aAssina[nXi,3]),oFontDad2,100)                        
            oPrint:Say(nLinha+230,1230,aAssina[nXi,4],oFontDad2,100)           
            oPrint:SayBitmap(nLinha+260,1230,aAssina[nXi,2],300,120)
       Case nXi == 5
            If aAssina[nXi,1]== "NãO SE APLICA"  .or. aAssina[nXi,1]=='Não Aprovado'
               oPrint:Say(nLinha+030,1630,aAssina[nXi,1],oFontDad2,100)
            Else
               oPrint:Say(nLinha+005,1630,Left(aAssina[nXi,1],12),oFontDad2,100)
               oPrint:Say(nLinha+055,1630,Alltrim(SubStr(aAssina[nXi,1],13,Len(aAssina[nXi,1]))),oFontDad2,100)
            EndIf
            oPrint:Say(nLinha+130,1633,OemToAnsi("DATA: ")+Dtoc(aAssina[nXi,3]),oFontDad2,100)
            oPrint:Say(nLinha+230,1630,aAssina[nXi,4],oFontDad2,100)           
            oPrint:SayBitmap(nLinha+260,1633,aAssina[nXi,2],300,120)
       Case nXi == 6
            If aAssina[nXi,1]== "NãO SE APLICA"  .or. aAssina[nXi,1]=='Não Aprovado'
               oPrint:Say(nLinha+030,1980,aAssina[nXi,1],oFontDad2,100)
            Else
               oPrint:Say(nLinha+005,1980,Left(aAssina[nXi,1],09),oFontDad2,100)
               oPrint:Say(nLinha+055,1980,Alltrim(SubStr(aAssina[nXi,1],10,Len(aAssina[nXi,1]))),oFontDad2,100)
            EndIf
            oPrint:Say(nLinha+130,1982,OemToAnsi("DATA: ")+Dtoc(aAssina[nXi,3]),oFontDad2,100)
            oPrint:Say(nLinha+230,1980,aAssina[nXi,4],oFontDad2,100)           
...
oPrint:Say(nLinha,0060,OemToAnsi("ACEITE DO PEDIDO"),oFontCab,100) 
nLinha += 50
...
While !(cAliasSCR)->(Eof())
   ...
   Do Case
      Case (cAliasSCR)->CR_NIVEL == '01' 
           aRetAss[1,1] :=  OemToAnsi("GERENTE DE PROJETO") // Titulo
      ...
      Case (cAliasSCR)->CR_NIVEL == '03' 
           aRetAss[3,1] :=  OemToAnsi("SUPRIMENTOS") // Titulo
