...
Total:=0
While ! Eof()
	// Tratamento para desconsiderar os pedidos de remessa. // Gilson 09/11/11
	If SC6->C6_TES $ GetNewPar("NM_TESNFL","670")
		SC6->(dbSkip())
		Loop
	EndIf
	...
	***************************************************************************
	...
	// Adicionado Por Leandro - CN Tecnologia em 26/02/2014 para Separação dos Reembolsos (Produto GSP0026) dos
	// outros pedidos de venda
	
	if (Alltrim(SC6->C6_PRODUTO) == 'GSP0026')
		Replace	Z0_HIST		With "NOTA DE DEBITO" + "- PED " + SC6->C6_NUM ,;
		Z0_LINHA	With "ND"
	else
		Replace	Z0_HIST		With "PEDIDOS DE VENDA" + "-" + SC6->C6_NUM ,;
		Z0_LINHA	With "PV"
	endif
	
	Replace 	Z0_FILIAL	With SC6->C6_FILIAL,;
	Z0_LOTE		With "9700"             ,;
	Z0_DOC		With "9700"+cSeq
	
	// retirados por causa do if nas linhas acima - Leandro CN
	...
	Else
		...
		Replace    Z0_VALOR 	 With Iif(SC5->C5_TIPOFAT $ "5/6",-(SC6->C6_QTDEMP*SC6->C6_PRCVEN),SC6->C6_QTDEMP*SC6->C6_PRCVEN) //CRISLEI TOLEDO - 04/10/07
	Endif
	...
	*/
	...
	If SC6->C6_TES <> "521"
		...
		Reclock("SZ0",.T.)
		   Replace Z0_FILIAL  With SC6->C6_FILIAL ,;
		           Z0_LINHA 	 With "PV" ,;
		           Z0_HIST 	 With "PED. VENDA ("+SC5->C5_NUM+Iif(Empty(SC6->C6_PIMP),")",") PIS COFINS ISS"),;
		           Z0_LOTE 	 With "9700"      ,;
		           Z0_DOC 	     With "9700"+cSeq ,;
		           Z0_VALOR 	 With -(SC6->C6_Valor*(SC6->C6_PIMP/100)),; //-(SC6->C6_Valor*Iif(!Empty(SC6->C6_PIMP),(SC6->C6_PIMP/100),0.1125)),;
